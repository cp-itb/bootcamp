...
protected:
    ...
    void TestCases() {
        ...
        for (int i=1; i<=3; i++){
            CASE(N = 6, M = 6, generateGrid(N,M,50,grid));
        }
        // medium tc
        for (int i=1; i<=2; i++){
            CASE(N = rnd.nextInt(1, 50), M = rnd.nextInt(1, 50), generateGrid(N,M,30,grid));
            CASE(M = rnd.nextInt(1, 50), N = rnd.nextInt(1, 50), generateGrid(N,M,50,grid));
        }
        // large tc
        for (int i=1; i<=2; i++){
            CASE(N = rnd.nextInt(1, MAXN), M = rnd.nextInt(1, MAXM), generateGrid(N,M,30,grid));
            CASE(M = rnd.nextInt(1, MAXN), N = rnd.nextInt(1, MAXM), generateGrid(N,M,50,grid));
        }
        for (int i=1; i<=3; i++){
